(() => {
  ...
  const copy = {
    de: {
      ...
      searchPlaceholder: "TV- oder Radiosender suchen", typeAll: "TV & Radio", typeTV: "TV-Sender", typeRadio: "Radiosender", resultsAll: "Programme gefunden", resultsTV: "TV-Sender gefunden", resultsRadio: "Radiosender gefunden", mediaTV: "TV", mediaRadio: "Radio", finderHint: "Erweitertes Line-up: 264 TV-Sender und 202 Radiosender.",
    ...
    en: {
      ...
      searchPlaceholder: "Search TV or radio", packageAll: "All packages", langAll: "All languages", categoryAll: "All categories", typeAll: "TV & radio", typeTV: "TV channels", typeRadio: "Radio stations", resultsAll: "services found", resultsTV: "TV channels found", resultsRadio: "radio stations found", mediaTV: "TV", mediaRadio: "Radio", reset: "Reset", resultsLabel: "services found", finderHint: "Extended line-up: 264 TV channels and 202 radio stations.", showMore: "Show more ↓",
    ...
    if (channelGrid) {
      channelGrid.innerHTML = filtered.length ? filtered.slice(0,visibleChannels).map((channel) => {
        ...
        return `<article class="channel-card channel-card--${channel.type || "tv"}">
          <span class="channel-card-logo${logo ? "" : " is-fallback"}${channel.logoTone === "light" ? " has-light-logo" : ""}">${logo ? `<img src="${logo}" alt="${channel.name}" loading="lazy">` : `<span>${channel.badge || channel.name.slice(0,2)}</span>`}</span>
